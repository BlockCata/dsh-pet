...
> 注：`video/` 源 mp4 不入 git，托管在 GitHub Releases 的 `assets-videos.zip`（全部源视频打包，浏览器直接下载）。`dsh-pet/assets/preview/` GIF 仅保留本機，不在本儲存庫追蹤；上游專案頁仍可提供動畫預覽，桌面版則直接使用 `desktop-app/assets/` 的 WebM 播放素材。
